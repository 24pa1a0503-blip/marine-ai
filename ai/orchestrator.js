 ...
 */
export async function processQuery(userQuery, options = { executeTools: true, verbose: true }, initialContext = {}) {
  ...
  if (options.verbose) {
    ...
    console.log(`   ↳ Language: ${intentResult.language}, Location Req: ${intentResult.locationRequired}, Dest Req: ${intentResult.destinationRequired}`);
  }
  ...
  if (options.verbose) {
    console.log(`   ↳ Generated Task Sequence: [${planResult.tasks.join(', ')}]`);
  }
  ...
  const updatedContext = contextMgr.updateFromQueryAndIntent(intentResult, toolResults);

  // Step 5: Response & Advisory Synthesis Agent
  if (options.verbose) console.log("🗣️ [Step 4] Synthesizing Evidence-Based Answer...");
  const synthesis = await synthesizeResponse(intentResult, planResult, toolResults, userQuery, updatedContext);
