...
async function runTests() {
  ...
  for (let i = 0; i < TEST_CASES.length; i++) {
    ...
    try {
      const result = await processQuery(testCase.query, { executeTools: true, verbose: false });
      ...
      const hasEvidence = Boolean(result.evidence && result.evidence.source && Array.isArray(result.evidence.parametersUsed));
      ...
      if (intentMatch && tasksMatch && langMatch && hasAnswer && hasRecommendation && hasEvidence) {
        ...
        if (!langMatch) console.log(`  Expected Language: ${testCase.expectedLang}, got: ${result.language}`);
