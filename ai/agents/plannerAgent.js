...
export async function createPlan(intentResult, userQuery = '') {
  const intent = typeof intentResult === 'string' ? intentResult : intentResult.intent;
  ...
  if (apiKey && apiKey.trim() !== '' && apiKey !== 'your_gemini_api_key_here') {
    try {
      ...
      if (parsed && Array.isArray(parsed.tasks)) {
        ...
        const validTasks = parsed.tasks.filter(t => AVAILABLE_TOOLS.includes(t));
        return {
          intent: intent,
          tasks: validTasks,
      ...
      console.warn("  [PlannerAgent] LLM API call failed or key invalid. Switching to deterministic planner. Error:", err.message);
    }
  }
